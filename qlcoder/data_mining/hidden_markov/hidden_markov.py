...
for line in open('145835459995807.txt', 'r'):
    tokens = line.split(',')
    alpha = my_ord(tokens[0])
    digit = my_ord(tokens[1])
    count = int(tokens[2])
...
for line in open('145835462696852.txt', 'r'):
    ...
    while idx < line_size:
        n1 = my_ord(line[idx - 1])
        n2 = my_ord(line[idx])
        trans[my_ord2(n1, n2)] += 1
        idx += 1
...
(first, second) = find_two(my_ord(instr[0]), my_ord(instr[1]))
outstr += chr(first + 97)
...
def find_one(p, d):
    max_prob, max_c = 0, 0
    for c in range(26):
        prob = freqs[d][c] * trans[my_ord2(p, c)]
        if max_prob < prob:
